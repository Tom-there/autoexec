...
pacman --noconfirm --needed -Sy dialog || error "Are you sure this is an arch distro and you are logged in as root? If yes, check your internet connection"

# 1.2 welcome
welcome(){
  dialog --title "Welcome!" --msgbox "This script is designed to install everything you need to be productive!" 10 50
  dialog --title "Welcome!" --yes-button "Lets go!" --no-button "wait..." --yesno "Are you ready to set up your user?" 10 50
  return $?
}
welcome || usererror
...
refreshing(){
  dialog --title "installing..." --infobox "Refreshing arch keyring" 10 50
...
getusername(){
  username=$(dialog --inputbox "First, enter your desired username." 10 50 3>&1 1>&2 2>&3 3>&1)
  while ! echo "$username" | grep -q "^[a-z_][a-z0-9_]*$"; do
    username=$(dialog --nocancel --inputbox "Username not valid.\nPlease enter valid username that matches RegEx: '[a-z_][a-z0-9_]*'" 10 50 3>&1 1>&2 2>&3 3>&1)
  done
}
...
getpassword(){
  userpass1=$(dialog --nocancel --passwordbox "What would you like as your password?" 10 50 3>&1 1>&2 2>&3 3>&1)
  userpass2=$(dialog --nocancel --passwordbox "Retype password." 10 50 3>&1 1>&2 2>&3 3>&1)
  while ! [ "$userpass1" = "$userpass2" ]; do
    unset userpass2
    userpass1=$(dialog --nocancel --passwordbox "Passwords did not match. Try again" 10 50 3>&1 1>&2 2>&3 3>&1)
    userpass2=$(dialog --nocancel --passwordbox "Retype password." 10 50 3>&1 1>&2 2>&3 3>&1)
  done
}
